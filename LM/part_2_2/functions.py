...
def pre_preparation_train(BOOL_ASGD,EMB_SIZE,HID_SIZE,VOCAB_LEN,LR,DEVICE):
    ...
    model = MODEL_LSTM(EMB_SIZE, HID_SIZE, VOCAB_LEN(lang),pad_index=lang.word2id["<pad>"],weight_tyining=True, variational_dropout = False).to(DEVICE)
...
def train_part(TRAINING,N_EPOCHS,DEVICE,CLIP,optimizer,model,train_loader,dev_loader,test_loader,criterion_eval,criterion_train):
    if TRAINING:
