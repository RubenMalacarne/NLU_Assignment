...
class MODEL_LSTM(nn.Module):
    def __init__(self, emb_size, hidden_size, vocab_size,weight_tyining = False,variational_dropout = False , pad_index=0, out_dropout=0.1,
    ...
    def forward(self, input_sequence):
        ...
        if self.variational_dropout:
            emb = self.emblin(emb) #usato un linear al posto di un dropout
            lstm_out = VariationalDropout(emb)
        else:
            emb = self.embedding(input_sequence)
